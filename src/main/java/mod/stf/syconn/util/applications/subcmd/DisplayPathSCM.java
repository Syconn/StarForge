...
import mod.stf.syconn.network.messages.MessageShowPath;
import mod.stf.syconn.network.messages.MessageShowShip;
import mod.stf.syconn.util.applications.cmd.DisplayCMD;

public class DisplayPathSCM extends BasicSCM<DisplayCMD> {

    private boolean enabled;
    private int gof;

    public DisplayPathSCM(DisplayCMD parentCMD) {
        super(parentCMD, "path");
        if (level.getBlockEntity(parentCMD.getApplication().getPos()) instanceof NavBE)
            enabled = ((NavBE) level.getBlockEntity(parentCMD.getApplication().getPos())).showPath();
        gof = 0;
    }
    ...
    protected CommandStatus hasParameters(String[] cmd) {
        if (cmd[0].matches(name)){
            ...
            return new CommandStatus("Rendering Path: " + enabled, CommandStatus.Status.SUCCESS);
        }
    ...
    @Override
    public void execute() {
        Network.getPlayChannel().sendToServer(new MessageShowPath(enabled, parentCMD.getApplication().getPos()));
    }
}
