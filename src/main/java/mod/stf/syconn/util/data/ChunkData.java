...
public class ChunkData {
    ...
    private final List<BlockInChunkData> chunks = new ArrayList<>();
            ...
            for (int z = 0; z < 16; z++) {
                for (int y = lowestY; y < highestY + 1; y++) {
                    ...
                    if (renderAble(pos, Minecraft.getInstance().level)) {
                        chunks.add(new BlockInChunkData(levelchunk.getBlockState(pos), pos, x, z));
                    }
                }
            }
        }
    ...
    public List<BlockInChunkData> getChunks() {
        return chunks;
    }
}
