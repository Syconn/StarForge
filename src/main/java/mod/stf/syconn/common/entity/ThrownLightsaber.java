...
public class ThrownLightsaber extends ThrowableItemProjectile {

    private static final EntityDataAccessor<ItemStack> DATA_ITEM_STACK = SynchedEntityData.defineId(ThrownLightsaber.class, EntityDataSerializers.ITEM_STACK);
    public int spin;
    private boolean returning = false;

    public ThrownLightsaber(LivingEntity entity, Level level, ItemStack item) {
        super(ModEntities.LIGHTSABER.get(), entity, level);
        this.getEntityData().set(DATA_ITEM_STACK, item.copy());
    }

    public ThrownLightsaber(EntityType<ThrownLightsaber> type, Level level) {
        super(type, level);
    ...
    public void tick() {
        ...
        if (entity != null) {
            ...
            if (returning) {
                ...
                this.setDeltaMovement(this.getDeltaMovement().scale(0.75D).add(vec3.normalize().scale(0.8D)));
            }
    ...
    protected void onHitEntity(EntityHitResult pResult) {
        ...
        if (!entity.is(getOwner()) && entity.hurt(entity.damageSources().source(ModDamage.LIGHTSABER, this, getOwner()), 8.0F)) {
            ...
            playSound(ModSounds.LIGHTSABER_SWING.get(), 0.8f, 1.0f);
        }
    }
